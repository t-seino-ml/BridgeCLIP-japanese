# -*- coding: utf-8 -*-
"""
Llama ゼロショット分類（Ollama API経由）

Ollama でビジョン対応Llamaモデルを動かし、画像分類を行う。

事前準備:
    # Ollama のインストール
    curl -fsSL https://ollama.com/install.sh | sh

    # ビジョン対応モデルのダウンロード（いずれか1つ）
    ollama pull llama3.2-vision        # 11B
    ollama pull llama3.2-vision:11b    # 11B（明示指定）
    ollama pull llama3.2-vision:90b    # 90B（高精度）

    # Ollamaサーバーを起動
    ollama serve

使い方:
    python -m classification.models.llama \
        --csv  classification/results/labeled_val.csv \
        --out  classification/results/llama_preds.csv \
        --n    50
"""
...
class LlamaClassifier:
    """
    Llama (Ollama) を使ったゼロショット分類器。

    Args:
        model:      Ollama のモデル名（デフォルト: llama3.2-vision）
        api_url:    Ollama API の URL
    ...
    def __init__(
        ...
        assert _REQUESTS_AVAILABLE, "requests ライブラリが必要です: pip install requests"
        self.model = model
    ...
    def _check_connection(self) -> None:
        ...
        try:
            ...
            print(f"[警告] Ollamaサーバーに接続できません: {e}")
            print("  ollama serve を起動してください。")
    ...
    def predict(self, image_path: str) -> dict[str, list[str]]:
        ...
        for attempt in range(RETRY_LIMIT):
            try:
                ...
                print(f"  [Llama] エラー (attempt {attempt+1}/{RETRY_LIMIT}): {e}")
...
def run_on_csv(
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Llama 推論"):
...
def main():
    parser = argparse.ArgumentParser(description="Llama ゼロショット分類")
